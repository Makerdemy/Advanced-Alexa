...
const questionintentHandler = {
  ...
  async handle(handlerInput) {
    ...
    var thing = handlerInput.requestEnvelope.request.intent.slots.things.resolutions.resolutionsPerAuthority[0].values[0].value.name;
    const attributesManager = handlerInput.attributesManager;
    const attributes = await attributesManager.getPersistentAttributes() || {};
    if(attributes.housekeys && thing === "housekeys")
    {
      var speechText = `You said your House keys are in the ${attributes.housekeys}`;
    }
    else if(attributes.officekeys && thing === "officekeys")
    {
      speechText = `You said your Office keys are in the ${attributes.officekeys}`;
    }
    else if(attributes.officekeys && thing === "purse")
    {
