...
const rp = require('request-promise');
  ...
  const requestOptions = {
    ...
    headers: {
      'X-CMC_PRO_API_KEY': '<please enter you API key here>'
    }
  };
...
const WhatCanIBuyIntentHandler = {
  ...
  async handle(handlerInput) {
    ...
    const speechText = `You can buy ${products.inSkillProducts[1].name} pack to get successful stories
    about cryptocurrencies and also you can buy ${products.inSkillProducts[0].name} pack to get tips 
...
const BuyResponseHandler = {
  ...
  async handle(handlerInput) {
    var speechText;
     ...
     if(productId === products.inSkillProducts[1].productId)
     {
      speechText = "You have unlocked successful stories, Now ask tell me a successful story";
     }
     else if(productId === products.inSkillProducts[0].productId)
     {
